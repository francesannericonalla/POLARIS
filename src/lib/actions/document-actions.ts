...
import { canUploadToUnit, canArchiveDocument } from "@/lib/permissions";
...
export async function archiveDocument(documentId: string) {
  ...
  if (!canArchiveDocument(profile, doc.unit_id)) throw new Error("Not authorized.");
...
export async function getSignedDownloadUrl(storagePath: string): Promise<string | null> {
  ...
  // Confirm this file actually belongs to a unit the caller can see
  ...
  const { data: doc } = await admin
    ...
    .single();
  if (!doc || !canArchiveDocument(profile, doc.unit_id)) return null;
